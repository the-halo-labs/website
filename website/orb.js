...
(() => {
  ...
  function currentAmp(){
    // live call → real audio energy from realtime.js (caller + agent mix)
    if (liveActive && window.__halo?.live){
      return window.__halo.live.getAmplitude();
    }
  ...
  // -------- LIVE MODE (OpenAI Realtime via realtime.js) --------
  let liveActive = false;

  // realtime.js reports state changes here.
  window.__halo = window.__halo || {};
  window.__halo.orb = {
    onLiveState(s){
      if (!liveActive) return;
      if (s === 'connecting')      setStatus('CONNECTING…', 'thinking');
      else if (s === 'listening')  setStatus('LISTENING', 'listening');
      else if (s === 'speaking'){  speakSynthT0 = performance.now(); setStatus('SPEAKING', 'speaking'); }
      else if (s === 'thinking')   setStatus('THINKING', 'thinking');
      else if (s === 'idle'){      liveActive = false; setStatus('STANDBY', 'idle'); }
    }
  };
  ...
  async function activate(){
    ...
    // Prefer the real agent when realtime.js is loaded + configured.
    if (window.__halo?.live){
      liveActive = true;
      setStatus('CONNECTING…', 'thinking');
      try {
        await window.__halo.live.connect('hero');
        return; // onLiveState drives everything from here
      } catch (e){
        console.warn('[orb] live agent unavailable, falling back to simulation:', e.message);
        liveActive = false;
      }
    }

    // Simulated fallback (original behavior).
  ...
  function deactivate(){
    ...
    if (liveActive && window.__halo?.live){
      liveActive = false;
      window.__halo.live.disconnect().catch(()=>{});
    }
    stopMic();
