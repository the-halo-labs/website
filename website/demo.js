...
(() => {
  ...
  const panel = {
    ...
    liveBtn: document.getElementById('demo-live'),
    liveLabel: document.getElementById('demo-live-label'),
    statusTxt: document.getElementById('demo-status-txt'),
  };

  // Track whether a live LiveKit call is active. When true, we suspend
  // scripted playback and let livekit.js drive transcript + waveform.
  let live = false;
  ...
  function reset(){
    ...
    panel.toggleLabel.textContent = 'Play sample';
  ...
  panel.toggle.addEventListener('click', () => {
    if (live) return;                     // scripted controls disabled while live
    if (playing) stop(); else start();
  });
  panel.reset.addEventListener('click', () => { if (!live) reset(); });
  panel.inds.forEach(b => b.addEventListener('click', () => {
    if (live) return;                     // industry lock during live call
    setIndustry(b.dataset.ind);
  }));

  // ---------- LIVE MODE (LiveKit) ----------
  async function startLive(){
    if (!window.__halo?.live) {
      addBubble('action', '▸ live agent not configured — check livekit.js');
      return;
    }
    // Stop scripted playback and clear the transcript so the real call
    // starts on a clean panel.
    stop();
    lineIdx = 0; tSeconds = 0;
    panel.transcript.innerHTML = '';
    panel.timer.textContent = '00:00';
    ...
    addBubble('action', '▸ connecting to Halo…');

    try {
      const info = await window.__halo.live.connect(current);
      addBubble('action', `▸ connected · industry: ${info.industry}`);
      setLiveUi('connected');
      // Live timer starts ticking now.
      timerId = setInterval(()=>{ tSeconds += 1; panel.timer.textContent = fmtTime(tSeconds); }, 1000);
    } catch (e) {
      addBubble('action', `▸ ${e.message || 'connection failed'}`);
      live = false;
      setLiveUi('idle');
    }
  }
  ...
  async function stopLive(){
    if (!live) return;
    try { await window.__halo.live.disconnect(); } catch(_){}
    // onLiveEnd() (called from livekit.js Disconnected event) handles UI cleanup.
  }

  function setLiveUi(mode){
    if (!panel.liveBtn) return;
    panel.liveBtn.classList.remove('is-connecting', 'is-live');
    if (mode === 'connecting'){
      panel.liveBtn.classList.add('is-connecting');
      panel.liveLabel.textContent = 'Connecting…';
      panel.toggle.setAttribute('disabled', '');
      panel.reset.setAttribute('disabled', '');
      if (panel.statusTxt) panel.statusTxt.innerHTML = 'CONNECTING · <span id="demo-timer">00:00</span>';
    } else if (mode === 'connected'){
      panel.liveBtn.classList.add('is-live');
      panel.liveLabel.textContent = 'End call';
      panel.toggle.setAttribute('disabled', '');
      panel.reset.setAttribute('disabled', '');
      if (panel.statusTxt) panel.statusTxt.innerHTML = '● LIVE · <span id="demo-timer">00:00</span>';
    } else {
      panel.liveLabel.textContent = 'Talk live';
      panel.toggle.removeAttribute('disabled');
      panel.reset.removeAttribute('disabled');
      if (panel.statusTxt) panel.statusTxt.innerHTML = 'CONNECTED · <span id="demo-timer">00:00</span>';
    }
    // Re-bind timer element (we rewrote it above).
    panel.timer = document.getElementById('demo-timer');
    panel.timer.textContent = fmtTime(tSeconds);
  }

  if (panel.liveBtn){
    panel.liveBtn.addEventListener('click', () => {
      if (live) stopLive(); else startLive();
    });
  }

  // Expose hooks for livekit.js to feed transcripts and UI events back in.
  window.__halo = window.__halo || {};
  window.__halo.demo = {
    addBubble,
    getCurrentIndustry: () => current,
    onLiveEnd(){
      clearInterval(timerId);
      live = false;
      setWaveActive(false);
      setLiveUi('idle');
      addBubble('action', '✓ call ended');
    },
    onLiveStateChange(s){
      // Optional: reflect listening/speaking states somewhere. Currently the
      // amplitude on the waveform is the main visual, so we no-op here.
    },
  };

  // auto-start when scrolled into view (only first time, and only if not live)
  let started = false;
  const io = new IntersectionObserver((entries)=>{
    entries.forEach(en => {
      if (en.isIntersecting && !started && !live){
        started = true;
        start();
      }
    });
  ...
  function drawWave(now){
    ...
    // When a live call is running, drive the wave with the real audio
    // amplitude (from livekit.js). Otherwise fall back to the scripted /
    // idle behaviour.
    const liveAmp = live ? (window.__halo?.live?.getAmplitude?.() ?? 0) : 0;
    const envScale = live
      ? (0.15 + liveAmp * 1.4)
      : (waveActive ? 1 : 0.12);
    const drawColor = (live || waveActive) ? acc : '#3a3a3f';
